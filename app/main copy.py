...
def compute_and_store_vam_peaks(db, activity, user_id: int):
    ...
    Met aussi à jour la table user_vam_prs si nouveaux records.
    """
    stream = (
    ...
    times = [p.elapsed_time for p in stream if p.elapsed_time is not None]
    alts = [p.altitude for p in stream if p.altitude is not None]
    if len(times) != len(alts) or len(times) < 2:
        print("⚠️ Données incomplètes pour calculer les VAM.")
        return

    def compute_vam_for_window(window_min):
    ...
    for window in [5, 15, 30]:
        ...
        loss = 0.0
        if gain > 0:
            loss_pct = max(0.0, (loss / gain) * 100.0)
        else:
            loss_pct = 0.0
        start_ts = stream[i].timestamp
        end_ts = stream[j].timestamp

        vam_peak = models.ActivityVamPeak(
            user_id=user_id,
            activity_id=activity.id,
            sport=activity.sport,
            window_min=window,
            max_vam_m_per_h=max_vam,
            start_idx=i,
            end_idx=j,
            ...
            gain_m=gain,
            loss_m=loss,
            loss_pct_vs_gain=loss_pct,
            distance_m=None,
            method="simple",
        )
        db.add(vam_peak)
        db.commit()
        _update_user_vam_pr(db, user_id, activity.sport, window, max_vam, activity.id, start_ts)
...
# -----------------------------------------------------------------------------
# Orchestrateur : enrichir une activité Strava (UTC aware partout)
# -----------------------------------------------------------------------------
async def enrich_activity(activity_id: int, user_id: int = 1):
    ...
    try:
        ...
        # 1) Activité Strava -> bornes temps (aware UTC)
        ...
        # 7) Mise à jour Strava + persistance du block
